...
function updateGsosPanelLang() {
  ...
  if (btnLabel)   btnLabel.textContent   = isEn ? "What's this in English?" : '다른 표현은 뭐가 있을까?';
  if (panelTitle) panelTitle.textContent = isEn ? 'SOS Word Helper 🚨' : '다양한 표현 도우미 ✨';
  if (hint) hint.innerHTML = isEn
    ...
    : '표현을 더 멋지게 바꿔보고 싶나요? 단어나 문장을 써보세요!<br>결과를 클릭하면 글쓰기 창에 바로 입력돼요 ✍️';
  if (askLabel)   askLabel.textContent   = isEn ? 'Ask!' : '물어보기';
  if (placeholder) placeholder.textContent = isEn ? 'Type a Korean word here 🔍' : '바꾸고 싶은 표현을 써보세요 🔍';
  if (input)      input.placeholder = isEn ? 'e.g. 사과하다, 그리움...' : '예) 정말 빨리, 신기했다...';
}
...
async function askGlobalSos() {
  ...
  try {
    const isEn = (_currentLang === 'en');
    const system = isEn
      ? `You are a bilingual English-Korean vocabulary helper for Korean elementary school students.
...
- Use the simplest possible vocabulary for elementary school level`
      : `당신은 한국 초등학생(8~12세)을 위한 글쓰기 표현 도우미입니다.
학생이 일기나 글에서 사용한 한글 단어/문구를 보면, 같은 뜻이지만 더 생생하고 구체적인 다른 한국어 표현 1~2개를 추천해주세요 (유의어, 비유, 감각적인 표현 등).
반드시 아래 JSON 형식으로만 답하세요 (마크다운, 설명 금지):
{
  "results": [
    {
      "word": "<추천 표현>",
      "korean": "<이 표현의 느낌/차이점, 10자 이내>",
      "example": "<추천 표현을 사용한 한국어 예문, 10단어 이내>",
      "example_ko": ""
    }
  ]
}
...
규칙:
- 초등학생이 이해하고 실제로 쓸 수 있는 쉬운 표현만 추천
- 원래 단어와 의미는 비슷하지만 더 생생하거나 구체적인 표현일 것
- 최대 1~2개만 추천
- example_ko 필드는 항상 빈 문자열("")로 둘 것`;
